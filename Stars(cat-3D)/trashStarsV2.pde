...
PlanetData[] planets= {   //nom, taille, vitesse
  new PlanetData("head", 100, 15),
  new PlanetData("pizza", 10, 5 ),
  new PlanetData("bird", 100, 1),
  new PlanetData("cat", 118, 35),
  new PlanetData("heart", 100, 0.04),
  new PlanetData("saturn", 60, 2 ),
  new PlanetData("rocket", 1, 6),
  new PlanetData("rubik", 2, 45),
  new PlanetData("toilet", 100, 20)
};
...
public void displayScene(PeasyCam cam, int ID) {
  ...
  if (ID==0) {
    ...
    cam.lookAt(0, 0, 0, 200, 0); //x,y,z,distance,delay
  }
